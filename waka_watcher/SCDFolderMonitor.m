...
//
//
//  Based on DFFolderMonitor by Oliver Drobnik on 05.08.13.
//  Copyright (c) 2013 Cocoanetics. All rights reserved.
//

#import "SCDFolderMonitor.h"

@implementation SCDFolderMonitor {
    NSURL *_URL;
    SCDFolderMonitorBlock _block;
...
+ (SCDFolderMonitor *)folderMonitorForURL:(NSURL *)URL block:(SCDFolderMonitorBlock)block {
    return [[SCDFolderMonitor alloc] initWithURL:URL block:block];
}

- (instancetype)initWithURL:(NSURL *)URL block:(SCDFolderMonitorBlock)block {
    ...
    if (self) {
        _URL = URL;
        _block = [block copy];
        _queue = dispatch_queue_create("SCDFolderMonitor Queue", 0);
    }
