...
- (IBAction)addDirectory:(id)sender {
    ...
    panel.title = NSLocalizedString(@"Select directories to monitor", @"select directories to monitor window title");
    panel.message = NSLocalizedString(@"Select the directories that you are planning to monitor and submit to "
    ...
    [panel beginWithCompletionHandler:^(NSInteger result) {
        if (result == NSFileHandlingPanelOKButton) {
            dispatch_queue_t myQueue = dispatch_queue_create("com.scidsolutions.wakawatcher.url_loader", NULL);
            dispatch_async(myQueue, ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    // Update the UI
                    [self.directoryView insertItemsAtIndexes:newRows inParent:nil withAnimation:YES];
                });
